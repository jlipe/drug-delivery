...
def pk_intestine_oil_lapatinib(y1, y2, y3, y4, y5, y6, t):
    ...
    Koaq = Satoil / ((Chi * (14.4e-3 + y4)) + (Satw * 10 ** (-6)))

    #calculation of total initial oil surface area (cm2);
    Aoil = N * 3.14 * ((D0) ** 2) * (((FA0 - (y4 * Vaq)) / FA0) ** (2./3))
    ...
    if t < 14400:
        ...
        if y1 > 0:
            ...
            if (Satm - ((y3 * Kmw) / (Kmw + 1))) > 0:
                dy[0] = - (par * y1 ** (2/3) * (Dp * (Satw - (y3 / (Kmw + 1))) + \
                    Dm * (Satm - ((y3 * Kmw) / (Kmw + 1)))))
            else:
                dy[0] = 0


        #drug uptake in oil, y(2) is mass (mg)
        dy[1] = P * Aoil * y3 * 1e-6 #%-(y(2)/(V_oil*Koaq)))

        if y1 > 0:
            #y(3) is drug conc. in aqueous medium in the lumen (mg/m3)
            dy[2] = (par / V * ((y1 ** (2. / 3)) * (Dp * (Satw - (y3 / (Kmw + 1))) + \
                Dm * (Satm - ((y3 * Kmw) / (Kmw + 1))))) - \
            dy[1] / V - Agi * Pgi * (y3 / Vd))
        else:
            dy[2] = - (Agi * Pgi * y3 / Vd)
    ...
    if (FA0 - (y[3] * Vaq)) > 0:
            ...
            dy[3] = Kdig / Vaq * Aoil - Kinh * y4 - Vmax * y4 / (Km + y4)
    ...
    if t<10800:   #t is residence time (sec), 3 hours in human small intestine in fed state;
        ...
        dy[4] = (Agi * Pgi * y3 / Vd) - (kel * y5)
    else:
        dy[5] = - k_travel * y6
        dy[4] = (y6 * Pgi * y3 / Vd) - (kel * y5)
...
print pk_intestine_oil_lapatinib(1, 2, 3, 4, 5, 6, 200)
print np.zeros(6)
